...
use crate::{XResult, random::{stable, normal}, simulation::{prelude::*, Subordinator}, SimulationError};
use rayon::prelude::*;
...
impl GeneralizedLangevin {
    pub fn new(drift_func: fn(f64, f64) -> f64, diffusion_func: fn(f64, f64) -> f64, start_position: impl Into<f64>, alpha: impl Into<f64>) -> XResult<Self> {
        let start_position = start_position.into();
        let alpha = alpha.into();
        if alpha <= 0.0 || alpha > 2.0 {
            return Err(SimulationError::InvalidParameters("alpha must be in the range (0, 2]".to_string()).into());
        }
        Ok(Self { drift_func, diffusion_func, start_position, alpha })
    }

    pub fn simulate(&self, duration: impl Into<f64>, time_step: f64) -> XResult<Pair> {
        simulate_generalized_langevin(self.drift_func(), self.diffusion_func(), self.start_position(), self.alpha, duration, time_step)
    }
    ...
    pub fn raw_moment(&self, duration: impl Into<f64>, order: i32, particles: usize, time_step: f64) -> XResult<f64> {
        let traj = self.duration(duration)?;
        traj.raw_moment(order, particles, time_step)
    }
    
    pub fn central_moment(&self, duration: impl Into<f64>, order: i32, particles: usize, time_step: f64) -> XResult<f64> {
        let traj = self.duration(duration)?;
...
pub fn simulate_generalized_langevin(drift: fn(f64, f64) -> f64, diffusion: fn(f64, f64) -> f64, start_position: impl Into<f64>, alpha: impl Into<f64>, duration: impl Into<f64>, time_step: f64) -> XResult<Pair> {
    let duration = duration.into();
    let alpha = alpha.into();
    let num = (duration / time_step).ceil() as usize;
    let t = (0..=num).into_par_iter().map(|i| time_step * i as f64).collect::<Vec<_>>();
    let mut x = vec![0.0; num+1];
    x[0] = start_position.into();
    let noise = stable::sym_standard_rands(alpha, num)?;
    for i in 1..=num {
        x[i] = x[i-1] + drift(x[i-1], t[i-1]) * time_step + diffusion(x[i-1], t[i-1]) * noise[i-1] * time_step.powf(1.0 / alpha);
    }
    Ok((t, x))
}
...
impl SubordinatedLangevin {
    pub fn new(drift_func: fn(f64, f64) -> f64, diffusion_func: fn(f64, f64) -> f64, start_position: impl Into<f64>, alpha: impl Into<f64>) -> XResult<Self> {
        ...
        if alpha <= 0.0 || alpha >= 1.0 {
            return Err(SimulationError::InvalidParameters("alpha must be in the range (0, 1)".to_string()).into());
        }
        Ok(Self { drift_func, diffusion_func, start_position, alpha })
    }

    pub fn simulate(&self, duration: impl Into<f64>, time_step: f64) -> XResult<Pair> {
        simulate_subordinated_langevin(self.drift_func(), self.diffusion_func(), self.start_position(), self.alpha, duration, time_step)
    }
...
pub fn simulate_subordinated_langevin(drift: fn(f64, f64) -> f64, diffusion: fn(f64, f64) -> f64, start_position: impl Into<f64>, alpha: impl Into<f64>, duration: impl Into<f64>, time_step: f64) -> XResult<Pair> {
    let duration = duration.into();
    let alpha = alpha.into();
    let num = (duration / time_step).ceil() as usize;
    let t = (0..=num).into_par_iter().map(|i| time_step * i as f64).collect::<Vec<_>>();
    ...
    for i in 1..=num {
        ...
        x[i] = x[i-1] + drift(x[i-1], t[i-1]) * delta_t + diffusion(x[i-1], t[i-1]) * noise[i-1] * delta_t.sqrt();
    }
    Ok((t, x))
}
...
mod tests {
    ...
    fn test_simulate_generalized_langevin() {
        let langevin = GeneralizedLangevin::new(
            |x, _t| x,
            |_x, _t| 1.0,
            0.0, 1.7).unwrap();
    ...
    fn test_simulate_subordinated_langevin() {
        let langevin = SubordinatedLangevin::new(
            |x, _t| x,
            |_x, _t| 1.0,
            0.0, 0.5).unwrap();
        let (t, x) = langevin.simulate(1.0, 0.01).unwrap();
