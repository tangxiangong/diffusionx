...
pub trait DiscreteProcess: Send + Sync + Sized {
    ...
    fn mean(&self, num_step: u64, particles: u64) -> XResult<f64> {
    ...
    /// * `num_step` - The number of steps of the simulation.
    /// * `particles` - The number of particles.
    fn msd(&self, num_step: u64, particles: u64) -> XResult<f64> {
        let traj = self.step(num_step)?;
    ...
    /// * `particles` - The number of particles.
    fn raw_moment(&self, num_step: u64, order: i32, particles: u64) -> XResult<f64> {
        let traj = self.step(num_step)?;
    ...
    /// * `order` - The order of the moment.
    /// * `particles` - The number of particles.
    fn central_moment(&self, num_step: u64, order: i32, particles: u64) -> XResult<f64> {
        let traj = self.step(num_step)?;
...
pub trait DiscreteTrajectoryTrait: DiscreteProcess {
    ...
    fn step(&self, num_step: u64) -> XResult<DiscreteTrajectory<Self>> {
...
impl<'a, SP: DiscreteProcess> DiscreteTrajectory<'a, SP> {
    ...
    pub fn new(sp: &'a SP, num_step: u64) -> XResult<Self> {
        if num_step == 0 {
