...
pub trait PointProcess<T: RealExt = f64, V: FloatExt = f64>: Send + Sync {
    /// Get the starting position
    fn start(&self) -> T;

    /// Get the ending position
    fn end(&self, duration: V) -> XResult<T> {
    ...
    fn displacement(&self, duration: V) -> XResult<T> {
        ...
        let (t, x) = loop {
            ...
            if V::from(end_time).unwrap() >= duration {
                break (t, x);
            }
            num_step *= 2;
        };
    ...
    ///
    /// * `duration` - The duration of the simulation.
    fn simulate_with_duration(&self, duration: V) -> XResult<(Vec<V>, Vec<T>)>
    where
        Self: Sized,
    {
        let mut num_step = duration.ceil().to_usize().unwrap();
    ...
    fn mean(&self, duration: V, particles: usize) -> XResult<f64>
    where
        Self: Sized + Clone + PointTrajectoryTrait<T, V>,
    {
    ...
    /// * `duration` - The duration of the simulation.
    /// * `particles` - The number of particles.
    fn msd(&self, duration: V, particles: usize) -> XResult<f64>
    where
        Self: Sized + Clone + PointTrajectoryTrait<T, V>,
    {
        let traj = self.duration(duration)?;
    ...
    /// * `particles` - The number of particles.
    fn raw_moment(&self, duration: V, order: i32, particles: usize) -> XResult<f64>
    where
        Self: Sized + Clone + PointTrajectoryTrait<T, V>,
    {
        let traj = self.duration(duration)?;
    ...
    /// * `order` - The order of the moment.
    /// * `particles` - The number of particles.
    fn central_moment(&self, duration: V, order: i32, particles: usize) -> XResult<f64>
    where
        Self: Sized + Clone + PointTrajectoryTrait<T, V>,
    {
        let traj = self.duration(duration)?;
    ...
    /// * `order` - The order of the moment.
    /// * `particles` - The number of particles.
    fn frac_raw_moment(&self, duration: V, order: f64, particles: usize) -> XResult<f64>
    where
        Self: Sized + Clone + PointTrajectoryTrait<T, V>,
    {
        let traj = self.duration(duration)?;
    ...
    /// * `order` - The order of the moment.
    /// * `particles` - The number of particles.
    fn frac_central_moment(&self, duration: V, order: f64, particles: usize) -> XResult<f64>
    where
        Self: Sized + Clone + PointTrajectoryTrait<T, V>,
    {
        let traj = self.duration(duration)?;
    ...
    fn fpt(&self, domain: (T, T), max_duration: V) -> XResult<Option<V>>
    where
        Self: Sized + Clone,
    {
    ...
    /// * `duration` - The duration of the simulation.
    fn occupation_time(&self, domain: (T, T), duration: V) -> XResult<V>
    where
        Self: Sized + Clone,
    {
    ...
    fn simulate_with_step(&self, num_step: usize) -> XResult<(Vec<V>, Vec<T>)>;
}

/// Point process trajectory
#[derive(Debug, Clone)]
pub struct PointTrajectory<SP: PointProcess<T, V> + Clone, T: RealExt = f64, V: FloatExt = f64> {
    /// The point process
    pub(crate) sp: SP,
    /// The duration of the trajectory
    pub(crate) duration: Option<V>,
    /// The number of steps of the trajectory
    pub(crate) num_step: Option<usize>,
    _marker: std::marker::PhantomData<T>,
}

pub trait PointTrajectoryTrait<T: RealExt = f64, V: FloatExt = f64>:
    PointProcess<T, V> + Clone
{
    ...
    fn duration(&self, duration: V) -> XResult<PointTrajectory<Self, T, V>> {
    ...
    fn step(&self, num_step: usize) -> XResult<PointTrajectory<Self, T, V>> {
        let traj = PointTrajectory::with_step(self.clone(), num_step)?;
        Ok(traj)
    }
}

impl<SP: PointProcess<T, V> + Sized + Clone, T: RealExt, V: FloatExt> PointTrajectoryTrait<T, V>
    for SP
{
}

impl<SP: PointProcess<T, V> + Clone, T: RealExt, V: FloatExt> PointTrajectory<SP, T, V> {
    ...
    pub fn get_duration(&self) -> Option<V> {
        self.duration
    }
    ...
    ///
    /// * `duration` - The duration of the trajectory
    pub fn with_duration(sp: SP, duration: V) -> XResult<Self> {
        if duration <= V::zero() {
    ...
    pub fn simulate_with_duration(&self) -> XResult<(Vec<V>, Vec<T>)> {
    ...
    pub fn simulate_with_step(&self) -> XResult<(Vec<V>, Vec<T>)> {
